...
## Components
- **Controllers** (`odoo_oidc_provider/controllers/main.py`): Implements discovery, authorize, token, userinfo, JWKS, introspection, revocation; enforces HTTPS (configurable) and PKCE S256 (configurable). Authorization enforces `nonce` for OIDC (`openid`) requests per OIDC Core 3.1.2.1 and allows exact external redirects (local guard disabled) per 3.1.2.5.
- **Models**:
...
## Flows (simplified)
- **Authorization Code + PKCE**: `/oauth/authorize` validates client + redirect, enforces PKCE for public clients, prompts consent when needed, issues durable auth code with challenge + nonce (required for `openid`). `/oauth/token` checks code + PKCE verifier, issues access/refresh + optional ID Token.

...
## Components
- **Controllers** (`odoo_oidc_provider/controllers/main.py`): Implements discovery, authorize, token, userinfo, JWKS, introspection, revocation; enforces HTTPS (configurable) and PKCE S256 (configurable). Authorization enforces `nonce` for OIDC (`openid`) requests per OIDC Core 3.1.2.1 (configurable), and allows exact external redirects (configurable local guard) per 3.1.2.5.
- **Models**:
